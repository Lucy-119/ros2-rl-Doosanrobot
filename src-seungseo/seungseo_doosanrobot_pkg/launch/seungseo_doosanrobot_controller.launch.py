...
Describer:  
			...
			/src/seungseo_doosanrobot_pkg/config/simple_controller.yaml
...
def generate_launch_description():
	...
	xacro_file = get_package_share_directory('seungseo_doosanrobot_pkg') + '/description'+'/xacro/'+ robot_model +'.urdf.xacro'
	...
	spawn_entity_robot = Node(package     ='gazebo_ros', 
							  ...
							  arguments   = ['-entity', 'seungseo_doosanrobot_robot', '-topic', 'robot_description'],
							  output      ='screen')

	# Gazebo   
	#world_file_name = 'my_empty_world.world'
	#world = os.path.join(get_package_share_directory('seungseo_doosanrobot_pkg'), 'worlds', world_file_name)
	#gazebo_node = ExecuteProcess(cmd=['gazebo', '--verbose', world,'-s', 'libgazebo_ros_factory.so'], output='screen')


	'''
	# RViz
	rviz_config_file = get_package_share_directory('seungseo_doosanrobot_pkg') + "/rviz/view_config.rviz"
